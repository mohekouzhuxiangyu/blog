...
export default function AdminPage() {
  ...
  if (!authed) return <AuthPage mode={mode} setMode={setMode} onLogin={() => { sessionStorage.setItem(AUTH_KEY, "1"); setAuthed(true); }} />;
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          <div className="flex mb-6 border-b border-gray-200">
            <button
              ...
              className={`flex-1 pb-2 text-sm font-medium text-center transition-colors ${mode === "login" ? "text-gray-900 border-b-2 border-gray-900" : "text-gray-400 hover:text-gray-600"}`}
            >
              Login
            </button>
            <button
              onClick={() => { setMode("register"); setError(""); }}
              className={`flex-1 pb-2 text-sm font-medium text-center transition-colors ${mode === "register" ? "text-gray-900 border-b-2 border-gray-900" : "text-gray-400 hover:text-gray-600"}`}
            >
              Register
            </button>
          </div>
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent"
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent"
            onKeyDown={(e) => e.key === "Enter" && handleSubmit()}
          />
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-gray-900 text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
...
function Dashboard() {
      ...
      try {
        ...
        setFiles(
          data
            ...
            .map((f: any) => ({ name: f.name, slug: f.name.replace(/\.md$/, "") }))
  ...
  async function handleDelete(name: string, slug: string) {
    ...
    try {
      ...
      await fetch(`/api/github/repos/mohekouzhuxiangyu/blog/contents/content/${name}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{files.length} posts</p>
        </div>
      ...
      {loading && <div className="text-center py-12"><p className="text-sm text-gray-400">Loading posts...</p></div>}
